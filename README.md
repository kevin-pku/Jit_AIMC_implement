...
The system typically uses a NOR-Flash array with INT8 weight precision and **Hybrid INT10 Activaction** precision.For vector input in Matrix-Vector Multiplication (MVM), the vector is quantized and then separated into 2-bit signed MSB (digital) and 8-bit signed LSB slices. The LSB pass can be sampled multiple times (default 2) to average out thermal noise.
