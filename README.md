...
For vector input in Matrix-Vector Multiplication (MVM), the vector is quantized into Int16 and then separated into the most significant bits (MSB-8) and least significant bits (LSB-8). Both parts undergo the same analog algorithmic process, with a 4-bit overlap to model noise-budget redundancy (physical bus still two 8-bit slices, effective precision ≈12 bits).
