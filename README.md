...
The system typically uses a NOR-Flash array with INT8 weight precision.For vector input in Matrix-Vector Multiplication (MVM), the vector is quantized into INT10 and then separated into 6-bit signed MSB and 4-bit unsigned LSB slices. The MSB pass can be sampled multiple times (default 2) to average out thermal noise.
