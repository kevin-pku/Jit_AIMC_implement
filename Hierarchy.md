...
### Hybrid_W8A11（数模混合稀疏架构）

#### 设计目标
在模拟存算信噪比约束和受限数字算力下，实现 高能效的 **INT8 等效系统精度 (INT8-equivalent precision)**。该架构中权重为INT8动态范围，对动态范围要求更高的 激活向量act，则为INT11动态范围。为了提高在模拟计算中act的实际精度和信噪比，采用 **2-bit 稀疏数字 MSB** + **9-bit 模拟 LSB** 的混合路径，利用激活值的本征长尾分布特性，最大化能效比。

#### 动态机制：稀疏旁路与非零编码 (Zero-Skipping & Non-Zero Encoding)
本架构引入了 **稀疏控制逻辑**，对于每一组输入act（已经先经过99.8pct的动态clipping），INT11量化并进行数值范围判断（以补码为例，只要11位二进制数的前3位是全0 (000) 或全1 (111)，数据必然落在$[-256, 255]$，这是充要条件）：

1.  **Mode 0: 稀疏旁路 (Bypass Mode)**
    *   **触发条件**：当激活值落在 $[-256, 255]$ 区间（占数据总量 >92%）。
    *   **行为**：稀疏控制器判定 MSB 为 0，**直接关断数字 MSB 路径**，仅由模拟 LSB 核心输出结果。
    *   **零值表达**：数值 **0** 由 LSB 的 9-bit `000000000` 精确表达，此时 MSB 处于关断状态，不消耗数字功耗。

2.  **Mode 1: 非零编码 (Non-Zero Encoding)**
    *   **触发条件**：当激活值属于长尾离群值（Outliers），此时。
    *   **行为**：激活数字路径。由于“0”状态已由 Mode 0 处理，**2-bit MSB 负载无需包含“0”值**。我们将 2-bit 编码重定义为 4 个非零状态 **$\{-2, -1, 1, 2\}$**。
    *   在数字域完成简单移位和运算（由于MSB只有两位，这里的运算其实是对于权重的简单移位加法），等LSB数据被ADC采样后，与LSB数据再次在数字域做移位和
    *   特别注意：在 Mode 1 下，模拟阵列接收的 LSB 并不是原始数据（如果是补码）的低9位（Bits[8:0]），需要做一定的映射（这在具体芯片中通常由 DAC 码表或简单的数字加减处理，不影响架构，但一定要留意）

#### 动态范围重构 (Scale-512 Strategy)
配合 **Scale=512** 的步长与 $[-256, 255]$ 的 LSB 补码能力，实现INT11全域连续覆盖，又保留了更高的精度：

*   **中心区 (MSB OFF)**：直接输出 LSB $\rightarrow [-256, 255]$。
*   **正向扩展 (MSB=1，2)**：
    *   MSB=1: $512 + [-256, 255] \rightarrow [256, 767]$
    *   MSB=2: $1024 + [-256, 255] \rightarrow [768, 1279]$ (**覆盖 INT11 全范围**)
*   **负向保护 (MSB=-1，-2)**： 同理，向下覆盖，满足INT11动态范围要求。

#### 混合路径处理流程
1.  **动态权重量化**：权重映射到 $[-128, 127]$ (INT8 标准)。
2.  **模拟路径 (LSB)**：
    *   输入范围 $[-256, 255]$，利用 Nor-Flash 电流镜阵列计算。
3.  **数字路径 (MSB)**：
    *   仅对 Mode 1 的非零 MSB 执行高精度数字乘加。
4.  **ADC 量化**：
    *   对 LSB 通道累加结果进行 10-bit 量化，保留 1-bit 动态余量以抵消积分非线性 (INL) 误差。
5.  **无噪声放大重构**：
    y = 512 × MSB_digital + LSB_analog
    放大系数 512 作用于无噪声的数字结果，确保系统信噪比 (SNR) 仅由 LSB 路径决定。
...
## FID-50k 评估流程（注入噪声）
在推理时开启噪声模拟（此处示例为 5.0 LSB），其余位宽/采样配置需与硬件设计匹配。
